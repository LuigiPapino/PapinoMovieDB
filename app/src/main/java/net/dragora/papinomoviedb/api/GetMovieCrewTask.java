 ...
 */
public class GetMovieCrewTask extends BaseTask<List<Crew>> {
     ...
     * @param callbacks
     */
    public GetMovieCrewTask(ApiTaskCallbacks<List<Crew>> callbacks, String movieId, MovieDbHelper dbHelper) {
        super(callbacks);
    ...
    protected void background() {
        ...
        Type listType = new TypeToken<ArrayList<Crew>>() {
        }.getType();

        try {
            JSONObject json = new JSONObject(serializedCredits);
            if (json.has("crew")){
                result = new Gson().fromJson(json.getString("crew").toString(), listType );
            }
